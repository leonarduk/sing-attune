...
class TestNotationPolicyDefaults:
    def test_v1_defaults_match_issue_spec(self):
        assert V1_NOTATION_POLICY.max_subdivision == Fraction(1, 4)
    ...
    def test_subdivision_matches_smallest_allowed_duration(self):
        assert float(V1_NOTATION_POLICY.max_subdivision) == min(
            V1_NOTATION_POLICY.allowed_durations_beats
        )
